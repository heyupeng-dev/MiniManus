...
class OpenAILLM(LLM):
    ...
    async def invoke(
        ...
        """使用异步 OpenAI 客户端发起块响应（该步骤可以切换成流式响应）"""
        try:
            # 检测是否传递了工具列表
            if tools:
                ...
                # 未传递工具则删除 tools / tool_choice 等参数
            ...
            raise ServerRequestsError("调用 OpenAI 客户端向LLM发起请求出错")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        response = await llm.invoke([{"role": "user", "content": "你好！你是什么AI？"}])
        print(response)
